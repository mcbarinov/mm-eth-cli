...
from mm_eth_cli import calcs, print_helpers, rpc_helpers, utils, validators
...
class Config(BaseConfig):
    ...
    def final_validator(self) -> Self:
        ...
        if self.addresses_map:
            for tx in utils.load_tx_addresses_from_str(self.addresses_map):
        ...
        if self.addresses_from_file and self.addresses_to_file:
            self.txs.extend(
                ...
                for tx in utils.load_tx_addresses_from_files(self.addresses_from_file, self.addresses_to_file)
            )
        if not self.txs:
...
def run(
    ...
    *,
    ...
    utils.init_logger(debug, config.log_debug, config.log_info)
...
def _run_transfers(config: Config, *, no_receipt: bool, emulate: bool) -> None:
    ...
    logger.debug(f"config={config.model_dump(exclude={'private_keys', 'addresses_map'}) | {'version': utils.get_version()} }")
    utils.check_private_keys(config.from_addresses, config.private_keys)
